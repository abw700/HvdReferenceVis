...
# database info
db_config = {
    'endpoints': 'rv-harvard-capstone-pgdb.c5usqplnbvhe.us-east-1.rds.amazonaws.com',
    'port': 5432,
    'dbname': 'rvcapsrv1',
    'username': 'rvcap',
    'password': 'rWJ5is53',
...
SQLALCHEMY_DATABASE_URI = 'postgres://{}:{}@{}/{}'.format(db_config['username'], db_config['password'], db_config['endpoints'], db_config['dbname'])
...
conn.execute("CREATE TABLE IF NOT EXISTS journals (jid SERIAL PRIMARY KEY, name text NOT NULL)")
conn.execute("CREATE TABLE IF NOT EXISTS articles (pmid SERIAL PRIMARY KEY, title text NOT NULL, abstract text, pubyear integer, jid integer)")
conn.execute("CREATE TABLE IF NOT EXISTS citations (cid SERIAL PRIMARY KEY, apmid integer NOT NULL, bpmid integer NOT NULL)")
...
df_journal_exists = pd.read_sql('journals', con=conn)
df_journal = df_journal.merge(df_journal_exists, how='left', on='name')
df_journal = df_journal[df_journal['jid'].isna()][['name']]

# write journals to sql
df_journal.to_sql('journals', con=conn, index=False, if_exists='append')
print('Wrote {:} new journals to DB'.format(df_journal.shape[0]))
...
df_article = df_article[['pmid', 'title', 'abstract', 'pubyear', 'jid']]

# check if article already in db if it is, don't add
df_article['pmid'] = df_article['pmid'].astype(int)
df_article['title'] = df_article['title'].str.encode('utf-8')
df_article['abstract'] = df_article['abstract'].str.encode('utf-8')
article_exists = pd.read_sql('''SELECT pmid FROM articles''', con=conn)['pmid'].tolist()
df_article = df_article[~df_article['pmid'].isin(article_exists)]

# write articles to sql
df_article.to_sql('articles', con=conn, index=False, if_exists='append')
print('Wrote {:} new articles to DB'.format(df_article.shape[0]))

# write references to sql
df_ref.to_sql('citations', con=conn, index=False, if_exists='append')
print('Wrote {:} new citations to DB'.format(df_ref.shape[0]))

# d = pd.read_sql('Articles', con=conn)
