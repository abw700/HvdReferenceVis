...
def test_article(client):
    ...
    response = post_json(client, "/article", json_dict)
    assert response.status_code == 200
    # valid article
    response = client.get("/article/26272696")
    assert response.status_code == 200
    # invalid article
...
def test_article_citerange(client):
    # no json body, return 400
    response = client.post("/article/citation-range")
    assert response.status_code == 400
    # correct json body, return 200
    json_dict = {'min_cite': 0, 'max_cite': 10}
    response = post_json(client, "/article/citation-range", json_dict)
    assert response.status_code == 200
    # wrong json body, fall back to default, return 200
...
def test_incoming(client):
    # wrong id, return 404
    response = client.get("/article/1/incoming-citations")
    assert response.status_code == 404
    # correct id, return 200
    response = client.get("/article/26272696/incoming-citations")
    assert response.status_code == 200

# test outgoing citations
def test_outgoing(client):
    # wrong id, return 404
    response = client.get("/article/1/outgoing-citations")
    assert response.status_code == 404
    # correct id, return 200
    response = client.get("/article/26272696/outgoing-citations")
    assert response.status_code == 200

# test /graph
def test_graph(client):
    # no json body, return 400
    response = client.post("/graph")
    assert response.status_code == 400
    # correct json body, return 200
    json_dict = {'min_cite': 9, 'max_cite': 10}
    response = post_json(client, "/graph", json_dict)
    assert response.status_code == 200
    # correct json body, return 200
...
def test_graph_title(client):
    # no json body, return 400
    response = client.post("/graph/title")
    assert response.status_code == 400
    # correct json body, return 200
...
def test_graph_id(client):
    # no json body, return 400
    response = client.post("/graph/26272696")
    assert response.status_code == 400
    # correct json body, return 200
